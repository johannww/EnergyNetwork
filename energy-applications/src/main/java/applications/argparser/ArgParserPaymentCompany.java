...
public class ArgParserPaymentCompany extends ArgParser {

    @Override
    protected void addSpecificOptions(String[] args) {
        programName = "PaymentCompany.jar";
        header = "\n1) ";
        
        Option paymentCertPath = new Option("cert", "certificate", true, "path to the payment x509 MSP certificate");
        options.addOption(paymentCertPath);

        Option paymentPkPath = new Option("pk", "privatekey", true, "path to the payment private key");
        options.addOption(paymentPkPath);
