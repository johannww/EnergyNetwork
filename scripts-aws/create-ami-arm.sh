...
# South Pacific (Mumbai) Ubuntu 20.04 Server (arm) ami: ami-0a6638920f7143fb2

echo -e $blueback \# "starting instance with Amazon Ubuntu 20.04 Server --ARM--: ami-0a6638920f7143fb2" $resetvid
instanceId=$(aws ec2 run-instances --image-id ami-0a6638920f7143fb2 --count 1 --instance-type t4g.micro --key-name $keyPairName --security-group-ids $securityGroup --subnet-id $subnetId --output text --query 'Instances[0].InstanceId') && echo $instanceId
...
ssh -i $SCRIPT_DIR/EnergyNetworkAwsKeyPair.pem ubuntu@$publicDnsName << EOF
    ...
    echo 'export GOROOT=/usr/local/go' | sudo tee -a /etc/profile.d/gopaths.sh    
    source /etc/profile
...
# create AMI image
imageId=$(aws ec2 create-image --instance-id $instanceId --name "EnergyNetworkImageArm" --description "ARM AMI with all requirements to run applications or hyperledger fabric containers for the EnergyNetwork" --output text --query 'ImageId') && echo "The AMI ID is: $imageId"
